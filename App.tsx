...
// Компонент DashboardScreen
const DashboardScreen: React.FC<{ config: Config; setConfig: (config: Config) => void }> = ({ config, setConfig }) => {
  ...
  const fetchServerData = async () => {
    try {
      const nodeResponse = await axios.get(`${config.serverUrl}/api2/json/nodes`, {
      ...
      const vmResponse = await axios.get(`${config.serverUrl}/api2/json/nodes/${node.node}/qemu`, {
        headers: { Authorization: `PVEAPIToken=${config.apiToken}` },
        httpsAgent: { rejectUnauthorized: false },
      });
      setVmStatus(
        vmResponse.data.data.map((vm: any) => ({
          id: vm.vmid,
          name: vm.name,
          status: vm.status,
      ...
      const storageResponse = await axios.get(`${config.serverUrl}/api2/json/nodes/${node.node}/disks/list`, {
        headers: { Authorization: `PVEAPIToken=${config.apiToken}` },
        httpsAgent: { rejectUnauthorized: false },
      });
      setDisks(
        storageResponse.data.data.map((disk: any) => ({
          name: disk.dev,
  ...
  const handleVmAction = async (id: number, action: string) => {
    try {
      await axios.post(
                ...
                <View style={styles.diskProgressBackground}>
                  <View
                    style={[
                      ...
                      { width: `${(parseFloat(disk.used) / parseFloat(disk.size.split(' ')[0])) * 100}%` },
                    ]}
                  />
                </View>
              </View>
...
// Компонент NotificationsScreen
const NotificationsScreen: React.FC<{ config: Config }> = ({ config }) => {
...
const SettingsScreen: React.FC<{ config: Config; setConfig: (config: Config) => void }> = ({ config, setConfig }) => {
  const [serverUrl, setServerUrl] = useState<string>(config.serverUrl || '');
  const [apiToken, setApiToken] = useState<string>(config.apiToken || '');
  const [updateInterval, setUpdateInterval] = useState<string>(config.updateInterval?.toString() || '5');
  const [dailyReports, setDailyReports] = useState<boolean>(config.dailyReports || true);
  const [loadWarnings, setLoadWarnings] = useState<boolean>(config.loadWarnings || true);
  const [diskAlerts, setDiskAlerts] = useState<boolean>(config.diskAlerts || true);
